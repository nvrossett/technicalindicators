...
class FixedSizeLinkedList extends LinkedList {
    constructor(size, maintainHigh, maintainLow, maintainSum) {
        ...
        if (!size || typeof size !== "number") {
            throw new Error(("Size required and should be a number."));
        }
...
class SD extends Indicator {
    constructor(input) {
        ...
        const period = input.period;
        const priceArray = input.values;
        const sma$$1 = new SMA({ period, values: [], format: (v) => v });
        this.result = [];
        this.generator = (function* () {
            let tick;
            let mean;
            const currentSet = new FixedSizeLinkedList(period);
            tick = yield;
            let sd;
            while (true) {
...
class BollingerBands extends Indicator {
    constructor(input) {
        ...
        let sma$$1, sd$$1;
        this.result = [];
        sma$$1 = new SMA({ period, values: [], format: (v) => v });
        sd$$1 = new SD({ period, values: [], format: (v) => v });
        this.generator = (function* () {
            let result;
            let tick;
            let calcSMA;
...
class ATR extends Indicator {
    constructor(input) {
        ...
        const wema$$1 = new WEMA({ period, values: [], format: (v) => v });
        this.result = [];
        this.generator = (function* () {
            let tick = yield;
...
class ROC extends Indicator {
    constructor(input) {
        ...
        this.generator = (function* () {
            let index = 1;
            const pastPeriods = new FixedSizeLinkedList(period);
            let tick = yield;
            let roc;
            while (true) {
...
class Stochastic extends Indicator {
    constructor(input) {
        ...
        this.generator = (function* () {
            let index = 1;
            const pastHighPeriods = new FixedSizeLinkedList(period, true, false);
            const pastLowPeriods = new FixedSizeLinkedList(period, false, true);
            const dSma = new SMA({
                period: signalPeriod,
                values: [],
                format: (v) => v,
...
class TRIX extends Indicator {
    constructor(input) {
        ...
        const period = input.period;
        const format = this.format;
        const ema$$1 = new EMA({ period, values: [], format: (v) => v });
        ...
        this.generator = (function* () {
            let tick = yield;
            while (true) {
                const initialema = ema$$1.nextValue(tick);
...
class CCI extends Indicator {
    constructor(input) {
        ...
        const format = this.format;
        const constant = .015;
        const currentTpSet = new FixedSizeLinkedList(period);
...
class StochasticRSI extends Indicator {
    constructor(input) {
        ...
        this.generator = (function* () {
            const index = 1;
            const rsi$$1 = new RSI({ period: rsiPeriod, values: [] });
            const stochastic$$1 = new Stochastic({ period: stochasticPeriod, high: [], low: [], close: [], signalPeriod: kPeriod });
            const dSma = new SMA({
                period: dPeriod,
                values: [],
                format: (v) => v,
...
class Highest extends Indicator {
    constructor(input) {
        ...
        const period = input.period;
        this.result = [];
        const periodList = new FixedSizeLinkedList(period, true, false, false);
        this.generator = (function* () {
            let tick;
            let high;
            tick = yield;
...
class Lowest extends Indicator {
    constructor(input) {
        ...
        const period = input.period;
        this.result = [];
        const periodList = new FixedSizeLinkedList(period, false, true, false);
        this.generator = (function* () {
            let tick;
            let high;
            tick = yield;
...
class Sum extends Indicator {
    constructor(input) {
        ...
        const period = input.period;
        this.result = [];
        const periodList = new FixedSizeLinkedList(period, false, false, true);
        this.generator = (function* () {
            let tick;
            let high;
            tick = yield;
...
function fibonacciretracement(start, end) {
    const levels = [0, 23.6, 38.2, 50, 61.8, 78.6, 100, 127.2, 161.8, 261.8, 423.6];
    let retracements;
    if (start < end) {
        retracements = levels.map(function (level) {
            const calculated = end - Math.abs(start - end) * (level) / 100;
...
class IchimokuCloud extends Indicator {
    constructor(input) {
        ...
        const currentConversionData = new FixedSizeLinkedList(params.conversionPeriod * 2, true, true, false);
        const currentBaseData = new FixedSizeLinkedList(params.basePeriod * 2, true, true, false);
        const currenSpanData = new FixedSizeLinkedList(params.spanPeriod * 2, true, true, false);
        this.generator = (function* () {
            let result;
            let tick;
    ...
    constructor(input) {
        ...
        this.generator = (function* () {
            ...
            while (true) {
                ...
                const atr$$1 = atrProducer.nextValue(tick);
...
class ChandelierExit extends Indicator {
    constructor(input) {
        ...
        const dataCollector = new FixedSizeLinkedList(input.period * 2, true, true, false);
        this.generator = (function* () {
            let result;
            let tick = yield;
            let atr$$1;
